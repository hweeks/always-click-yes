...
// session or on one of the judge's one-shot sessions.
...
// server, the same generated hook settings, the same launcher, the same judge, the
// same persistence. Exported so the live e2e suite can drive the real thing rather
...
func NewSupervisor(ctx context.Context, f Flags) (*Supervisor, error) {
	...
	launcher := func(ctx context.Context, spec ui.LaunchSpec) (*driver.Driver, error) {
		...
		opts := driver.Options{Bin: f.Bin, Cwd: f.Cwd, Model: model, UseAPIKey: f.UseAPIKey}
		switch spec.Phase {
	...
	judgeFn := func(ctx context.Context, plan, lastMsg string) (judge.Result, error) {
		return judge.Assess(ctx, judge.Options{
			Bin: f.Bin, Cwd: f.Cwd, Model: judgeModel, UseAPIKey: f.UseAPIKey,
		}, plan, lastMsg)
	}
