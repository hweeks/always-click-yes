...
var ArchSystemPrompt = strings.Join([]string{
	...
	"",
	"The ticket board is this run's memory: " + mcp.Qualified(mcp.ToolReadTickets) + " it first, and after every",
	"merge. Keep statuses current with " + mcp.Qualified(mcp.ToolUpdateTicket) + " at every transition — launch",
	"to in-progress, PR opened to in-review, merged once the human merges it, or blocked with a note the",
	"moment an engineer is stuck. A resumed run has no memory of this conversation; the board is how it",
